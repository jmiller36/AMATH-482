...
for j = 1:numFrames1
    gray1(:,:,j) = rgb2gray(vidFrames1_4(:,:,:,j));
    if j == 1
        initialize(positionTracker, [381 317], gray1(:,:,1));
        initialize(noiseTracker, [20 20], gray1(:,:,1));
    end
...
for j = 1:numFrames2
    gray2(:,:,j) = rgb2gray(vidFrames2_4(:,:,:,j));
    if j == 1
        release(positionTracker);
        release(noiseTracker);
        initialize(positionTracker, [256 289], gray2(:,:,1));
        initialize(noiseTracker, [20 20], gray2(:,:,1));
    end
...
for j = 1:numFrames3
    gray3(:,:,j) = rgb2gray(vidFrames3_4(:,:,:,j));
    if j == 1
        release(positionTracker);
        release(noiseTracker);
        initialize(positionTracker, [400 210], gray3(:,:,1));
...
ylabel('Y Coordinate');
xlabel('Time');
title('Scenario 3, Camera 1 Y-Component');
print(gcf,'-dpng','3y.png');