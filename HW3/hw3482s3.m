...
for j = 1:numFrames1
    gray1(:,:,j) = rgb2gray(vidFrames1_3(:,:,:,j));
    if j == 1
        initialize(positionTracker, [340 340], gray1(:,:,1));
...
for j = 1:numFrames2
    gray2(:,:,j) = rgb2gray(vidFrames2_3(:,:,:,j));
    if j == 1
        release(positionTracker);
        release(noiseTracker);
        initialize(positionTracker, [254 352], gray2(:,:,1));
...
for j = 1:numFrames3
    gray3(:,:,j) = rgb2gray(vidFrames3_3(:,:,:,j));
    if j == 1
        release(positionTracker);
        release(noiseTracker);
        initialize(positionTracker, [405 242], gray3(:,:,1));
